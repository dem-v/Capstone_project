...
def parse_args() -> argparse.Namespace:
    ...
    add_split_arg(parser, default="train", help=None)
...
def main() -> None:
    ...
    progress.log("started CXR XAI threshold calibration", run_start)
    ...
    for sample_idx, row in enumerate(rows):
        ...
        progress.log(f"  case {sample_idx + 1}: Grad-CAM", run_start)
        ...
        progress.log(f"  case {sample_idx + 1}: Eigen-CAM", run_start)
        ...
        progress.log(f"  case {sample_idx + 1}: attributions done; sweeping fractions", run_start)
        ...
        progress.log(f"case {sample_idx + 1}/{len(rows)} | done | {case_filename}", run_start)

    gradcam.remove_hooks()
    progress.log("all cases processed; aggregating per-method/per-fraction summary", run_start)
    ...
    progress.log("calibration complete", run_start)
    progress.finish()
