...
spatial maps. For instance, \texttt{Grad-\allowbreak CAM} \cite{ref-gradcam} applies weights to
target-layer activations based on gradients of the selected class score, while
\texttt{Grad-\allowbreak CAM++} \cite{ref-gradcampp} uses a modified gradient-weighting
...
Figure 2.1 is intended to illustrate the conceptual risk that a heatmap can be visually plausible
while highlighting clinically indirect or confounded image regions
rather than the annotated lesion. The figure may use one
de-identified example from this study, drawn from the review workbook, or a schematic if
...
This work moved from a legacy v1 naming scheme with separate
...
performed in WSL Ubuntu, with the package installed editably from
...
\texttt{torchxrayvision==1.\allowbreak 4.\allowbreak 0}. AI-assisted development tools used
during the research workflow are disclosed in accordance with
...
shared package modules. A classifier-screening stage evaluates each
TorchXRayVision baseline on the test split and records image-level
ranking metrics. An explanation stage runs the attribution methods and
...
visualizations and outcome-balanced visualizations grouped by
classifier outcome. A final review stage assembles the structured
...
visualizations, true-positive, false-positive, true-negative, and
false-negative groupings are preserved as separate subfolders. The
complete manifest mapping every reported experiment to its run folder
is given in Appendix~\ref{experiment-configuration}, which serves as the
canonical record of the artifacts underlying Chapter 4.

Reproducibility is supported by an automated test suite covering the
package modules and output schemas, together with syntax checks on the
analysis stages; documentation-only changes are reviewed for scope and
wording but do not trigger the test suite.
...
co-primary TorchXRayVision follow-up: its full test screen gives higher
...
\texttt{mean\_\allowbreak dice=0.\allowbreak 0237}, \texttt{mean\_\allowbreak iou=0.\allowbreak 0130}).
\item The Stage A
...
not as a clinically strong model. In this comparison,
...
\item Thesis-safe wording: ResNet-50 showed a relative improvement over the
...
aggregate positive-view localization means from the Stage A diagnostic
sweep.
...
\texttt{grad\_\allowbreak cam\_\allowbreak plus\_\allowbreak plus}. In the ResNet-50 run, consensus
...
Key interpretation points for quantitative CXR explanation validation are as follows.
\begin{itemize}
\item
Quantitative overlap metrics and peak-localization metrics answer
different questions. Across the Stage A all-model correlations,
...
meaningful than lesion overlap.
...
be faithful to the model while still being clinically poorly localized.
\item For
...
target \texttt{1\ -\allowbreak \ P(normal)}.
...
below, it presents a representative visual example that connects the
...
\begin{figure}[htbp]
	...
	\hfill
	\includegraphics[width=0.237\linewidth,height=0.32\textheight,keepaspectratio]{figures/iter_48_resnet_review_workbook_balanced40_smoothed_faithfulness__review__assets__case_31__101_test_1__consensus_signed_continuous_heatmap.png}%
	\caption{Representative review-workbook case for explanation visual inspection}
	\label{figure:4-1}
\end{figure}

Key interpretation points from the balanced 40-case ResNet review
(scored 2026-05-25) are summarized below. This is the canonical
radiologist-review evidence for the thesis; the earlier 10-case smoothed
review is preserved as a methodological pilot but is superseded by the
balanced outcome-stratified 40-case scoring.
\begin{itemize}
...
\item Qualitative flags from the
...
findings.
\item Interpretation guard: device/tube and subcutaneous-emphysema
...
correlations on the balanced 40-case set are modest: the strongest absolute
...
Chart \ref{chart:4-2} visualizes Table 4.4 as review-score counts.
...
pointing-hit, and precision-at-fraction results were computed for both
runs and are retained with the experiment artifacts.

Key interpretation points for the improvement and visualization work are as follows.
\begin{itemize}
\item
...
inspectable, not necessarily more clinically correct.
\item Exploratory
...
constant across modalities; for this reason this thesis restricts
...
This appendix collects reproducibility information that would interrupt
the main text if placed in Chapter 3. It records the following:
...
\begin{itemize}
	...
	\item
	the historical v1/v2 output folders, which are preserved and not
	overwritten.
\end{itemize}

The command block below shows the template used to produce the final runs:

\begin{Shaded}
	\begin{Highlighting}[]
		\NormalTok{wsl.}\FunctionTok{exe}\NormalTok{ {-}{-}cd /path/to/project python3 scripts/run\_improvement\_experiment.}\FunctionTok{py}\NormalTok{ \textasciigrave{}}
...
The key commands and output artifacts for this experiment are listed below.
...
This appendix records the review-task instructions and scoring schema used
for the balanced review workbook. It makes the rater task auditable without
requiring the reader to open the HTML workbook, and covers the following:
...
\begin{itemize}
	...
	\end{itemize}
	\item
	the warmup-case procedure, with the review framed as a structured
	qualitative assessment rather than prospective clinical validation;
	\item
...
This appendix contains supporting visual examples that are useful but
too detailed for Chapter 4. It includes the following:
...
Every appendix figure records the source image stem in the
filename or caption, together with the model, method, view, classifier
probability, threshold/fraction, and whether the case is \texttt{tp}, \texttt{fp},
