...
\begin{figure}[htbp]
	...
	\subcaption{(a) Chest X-ray with the annotated pneumothorax mask (green contour) in the right lung apex. (b) Consensus attribution at the top 20\% selected fraction and (c) Grad-CAM positive evidence at the top 5\% selected fraction concentrate on central, mediastinal regions and largely miss the annotated lesion.}
	\label{figure:2-1}
\end{figure}
...
\texttt{tn}, or \texttt{fn}). The selection criteria use both the F1 score
and Youden's J statistic to identify the threshold candidate with the
best performance.
