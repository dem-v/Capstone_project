...
\begin{figure}[htbp]
	...
	\caption{Conceptual risk of visually plausible but clinically misleading saliency maps. (a) Chest X-ray with the annotated pneumothorax mask (green contour) in the right lung apex. (b) Consensus attribution at the top 20\% selected fraction and (c) Grad-CAM positive evidence at the top 5\% selected fraction concentrate on central, mediastinal regions and largely miss the annotated lesion.}
	\label{figure:2-1}
\end{figure}

Figure 2.1 illustrates the conceptual risk that a heatmap can be visually plausible
...
the selected target class. Figure 3.1 summarizes this end-to-end
validation pipeline.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.92\linewidth,height=0.46\textheight,keepaspectratio]{figures/methodology_pipeline__figure_3_1.png}
	\caption{End-to-end validation pipeline used in this thesis. A medical image with its label/mask is passed through a frozen classifier and target score, post-hoc XAI methods produce positive, negative, magnitude, and signed views, and these feed four parallel evaluation branches (calibrated top-fraction localization metrics, deletion/insertion faithfulness curves, signed-evidence and agreement diagnostics, and the radiologist-centered review workbook), whose outputs are interpreted separately for classifier behavior, localization, faithfulness, and usefulness.}
	\label{figure:3-1}
\end{figure}
...
The CXR pipeline now uses a classifier-loading seam, which returns the model,
...
\begin{itemize}
	...
	\begin{equation}
		P@f(A, M) = \frac{|P_f \cap M|}{|P_f|} = \frac{\sum_{(i,j) \in P_f} M_{i,j}}{|P_f|}
	\end{equation}
	\begin{center}
		\small
		\begin{tabular}{lp{9cm}}
...
\texttt{tn}, or \texttt{fn}). The selection criteria uses F1 score 
to identify the threshold candidate with the best performance.
...
In this work the author, as such radiologist with 5 years of 
experience at high-volume trauma center, executed a single-reader
...
example visualizations with masking) are reported, and the analysis
code associated with this work is intended for open release to the
research community, that will be available as a github repository. 
...
package modules and output schemas.
...
Table \ref{table:4-2} summarizes the already completed Stage A localization signal
...
(\texttt{0.\allowbreak 0411}) and \texttt{Grad \allowbreak CAM\allowbreak ++} (\texttt{0.\allowbreak 0409}),
with consensus close behind (\texttt{0.\allowbreak 0383}) and the remaining methods
in the \texttt{0.\allowbreak 026-\allowbreak 0.\allowbreak 036} range. For ResNet-50, \texttt{Grad \allowbreak CAM} has
the highest mean Dice (\texttt{0.\allowbreak 0540}), followed by \texttt{Score \allowbreak CAM}
...
over \texttt{Score\allowbreak CAM}, but it does not significantly beat the
stronger aggregate Dice methods such as \texttt{occlusion} or
\texttt{Grad \allowbreak CAM\_\allowbreak ++}. In the ResNet-50 run, consensus
significantly improves Dice/IoU over \texttt{Grad\_\allowbreak CAM\_\allowbreak ++},
\texttt{Integrated \allowbreak Gradients}, \texttt{Gradient \allowbreak SHAP},
\texttt{occlusion}, and \texttt{Eigen \allowbreak CAM}, but not over
\texttt{Grad \allowbreak CAM} or \texttt{Score \allowbreak CAM}. Therefore, consensus should
...
\texttt{GradientSHAP}), post-processing affects readability and 
...
\texttt{Integrated \allowbreak Gradients}, \texttt{Gradient\allowbreak SHAP}, and
...
gains mainly over \texttt{Score \allowbreak CAM}, while stronger aggregate Dice
...
\begin{longtable}[]{@{}llllll@{}}
	...
	\end{minipage} & \begin{minipage}[t]{0.14\columnwidth}\raggedright
		Dice/ IoU/ Precision higher than \texttt{Score\_\allowbreak CAM}; Pointing hit varies, 
	...
	\end{minipage} & \begin{minipage}[t]{0.14\columnwidth}\raggedright
		...
		\texttt{Grad \allowbreak CAM} or \texttt{Score \allowbreak CAM}\strut
	\end{minipage} & \begin{minipage}[t]{0.16\columnwidth}\raggedright
...
\begin{figure}[htbp]
	...
	\caption{Representative review-workbook case for explanation visual inspection, TP, correct}
	\label{figure:4-1}
\end{figure}
...
\texttt{devices \allowbreak or text \allowbreak artifacts} 2/40. The dominant failure mode at
...
Dice-focused paired comparison because out of Dice, IoU, pointing hit and precision at fraction, the former is the most interpretable overlap summary for selected attribution regions. 
...
\begin{graph}[htbp]
	...
	\vspace{1em} % Ersätter \vfill för kontrollerat avstånd mellan raderna
	...
	% --- HUVUDUNDERTEXT FÖR HELA GRAFEN ---
...
\begin{longtable}[]{@{}
	...
	\midrule
	\endhead
	\texttt{densenet121-\allowbreak res224-\allowbreak chex} & \texttt{Grad \allowbreak CAM} &
	...
	\midrule
	\texttt{densenet121-\allowbreak res224-\allowbreak chex} & \texttt{Grad \allowbreak CAM\allowbreak ++} &
	Consensus median: \texttt{0.0211}\newline
	...
	Interpretation: not significant\tabularnewline
	\midrule
	\texttt{densenet121-\allowbreak res224-\allowbreak chex} & \texttt{IG} &
	Consensus median: \texttt{0.0211}\newline
	...
	Interpretation: not significant\tabularnewline
	\midrule
	\texttt{densenet121-\allowbreak res224-\allowbreak chex} & \texttt{Gradient\allowbreak SHAP} &
	Consensus median: \texttt{0.0211}\newline
	...
	Interpretation: not significant\tabularnewline
	\midrule
	\texttt{densenet121-\allowbreak res224-\allowbreak chex} & \texttt{occlusion} &
	Consensus median: \texttt{0.0211}\newline
	...
	Interpretation: not significant\tabularnewline
	\midrule
	\texttt{densenet121-\allowbreak res224-\allowbreak chex} & \texttt{Eigen \allowbreak CAM} &
	Consensus median: \texttt{0.0211}\newline
	...
	Interpretation: not significant\tabularnewline
	\midrule
	\texttt{densenet121-\allowbreak res224-\allowbreak chex} & \texttt{Score \allowbreak CAM} &
	Consensus median: \texttt{0.0211}\newline
	...
	\midrule
	\texttt{resnet50-\allowbreak res512-\allowbreak all} & \texttt{Grad \allowbreak CAM} &
	...
	Interpretation: not significant\tabularnewline
	\midrule
	\texttt{resnet50-\allowbreak res512-\allowbreak all} & \texttt{Grad \allowbreak CAM\allowbreak ++} &
	Consensus median: \texttt{0.0218}\newline
	...
	\midrule
	\texttt{resnet50-\allowbreak res512-\allowbreak all} & \texttt{IG} &
	Consensus median: \texttt{0.0218}\newline
	...
	Interpretation: significant\tabularnewline
	\midrule
	\texttt{resnet50-\allowbreak res512-\allowbreak all} & \texttt{Gradient\allowbreak SHAP} &
	Consensus median: \texttt{0.0218}\newline
	...
	Interpretation: significant\tabularnewline
	\midrule
	\texttt{resnet50-\allowbreak res512-\allowbreak all} & \texttt{occlusion} &
	Consensus median: \texttt{0.0218}\newline
	...
	\midrule
	\texttt{resnet50-\allowbreak res512-\allowbreak all} & \texttt{Eigen \allowbreak CAM} &
	Consensus median: \texttt{0.0218}\newline
	...
	Interpretation: significant\tabularnewline
	\midrule
	\texttt{resnet50-\allowbreak res512-\allowbreak all} & \texttt{Score \allowbreak CAM} &
	Consensus median: \texttt{0.0218}\newline
...
\texttt{Score \allowbreak CAM} by Dice, IoU and precision, but it is not the best method
by aggregate mean Dice (\texttt{occlusion} and
\texttt{Grad \allowbreak CAM\allowbreak ++} are slightly higher descriptively). For
ResNet-50, consensus is statistically better than several methods by
Dice/IoU, but \texttt{Grad \allowbreak CAM} and \texttt{Score \allowbreak CAM} remain
...
\texttt{Score-\allowbreak CAM} channels or coarse occlusion patches; 
...
\texttt{Score \allowbreak CAM}, while \texttt{occlusion} and
\texttt{Grad \allowbreak CAM\allowbreak ++} remain slightly higher by aggregate mean
Dice. In the ResNet-50 baseline, consensus significantly improves
Dice/IoU over several weaker methods, but not over \texttt{Grad \allowbreak CAM} or
\texttt{Score \allowbreak CAM}. This supports a model-dependent conclusion:
