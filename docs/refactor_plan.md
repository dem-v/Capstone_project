...
| 7 | Phase 5.1 Eigen-CAM + Score-CAM | 0.5 | low | ✅ Done. Both registered as individual MethodSpecs and carried through calibration v3 + both held-out improvement experiments. |
| 8 | Phase 5.2 improvement experiment script + narrative pre-drafts | 1 | low | ✅ Done (script). `scripts/run_improvement_experiment.py` (calibrate→freeze→test, paired Wilcoxon/Holm/10k-bootstrap) run for DenseNet (`outputs/iter_51_densenet_improvement_v3/`) and ResNet-50 (`outputs/iter_52_resnet_improvement_v3/`). Narrative pre-drafting is handled by the student outside the agent loop. |
| 9 | Phase 5.4 CT pilot: **hour-1 model-availability check** then scaffold + first CT smoke (or fallback) | 2-3 | high | ✅ Done (Branch A, full pipeline). Gate passed (HF DifeiT ViT, `1−P(normal)` target verified byte-exact); `ct/io.py`+`ct/models.py`+manifest+smoke committed (`c0b3ffd`→`411a7cd`); calibrated held-out improvement experiment run (`outputs/iter_54_ct_improvement_test/`, `90c1be0`). CAM-family on the ViT deliberately excluded as future work (cited). |
...
| 12 | Phase 5.5 stronger second CXR model full protocol | 1-2 | medium | ✅ Done. ResNet-50 Stage A, calibration v2/v3, classifier-outcome 1000-case run, 40-case review, and the held-out improvement experiment (`outputs/iter_52_resnet_improvement_v3/`) are all complete. |
...
1. **`DifeiT/rsna-intracranial-hemorrhage-detection` on HuggingFace Hub** (recommended primary).
   ...
   - License: Apache 2.0 — **VERIFIED 2026-06-02** via the gate probe (`apache-2.0` on the hub card).
   - Class head: ⚠️ **CORRECTED 2026-06-02 (gate probe).** The pre-baked assumption of a 6-output set including a binary `any` head was WRONG. The actual checkpoint is `id2label = {0: epidural, 1: intraparenchymal, 2: intraventricular, 3: normal, 4: subarachnoid, 5: subdural}` with `problem_type = single_label_classification` (softmax over 6 mutually-exclusive classes). **There is no `any` head.** The binary-hemorrhage attribution target analogous to the CXR `Pneumothorax` head must therefore be derived: **hemorrhage = 1 − P(normal)** (attribute the negated `normal` logit, or an aggregate of the 5 subtype logits). This is an OPEN design decision for integration.
   - Loading: `from transformers import AutoModelForImageClassification, AutoImageProcessor; model = AutoModelForImageClassification.from_pretrained("DifeiT/rsna-intracranial-hemorrhage-detection"); processor = AutoImageProcessor.from_pretrained("...")` — confirmed working, `transformers` 5.9.0.
   - Input contract: ⚠️ **CORRECTED 2026-06-02.** NOT ImageNet normalization. The processor reports `image_mean = image_std = (0.5, 0.5, 0.5)` → input range [−1, 1], 224×224 RGB. Consequence: `brain_window_center_normalized = (0.5 − 0.5)/0.5 = 0.0`, so the `brain_window_center` faithfulness baseline is numerically identical to `zero_tensor` for this checkpoint (`black` = −1024 stays distinct). The deferred faithfulness branch is degenerate here; document the equivalence in Chapter 3. CT brain-window single-channel image is 3-channel-replicated (see `ct/io.py::to_three_channel`).
   - Verification command (hour-1): ✅ **RUN 2026-06-02** via `scripts/ct_gate_probe.py` — loads, one CPU forward on a random 224×224×3 image gives finite per-class probabilities, license clean. Model-track gate PASSES with the target-redefinition caveat above. Remaining hour-1 steps need real PhysioNet slices (mask track, user-owned).
