...
- The clinical-implausibility concern carried from Week 2 increased: many positive-evidence maps show attribution on ribs, mediastinum, costophrenic regions, or even outside the body, which is consistent with a baseline model that produces moderate classifier signal from broad cues rather than localized pneumothorax features.
...
Iteration 22 ran a Random Train-100 XAI calibration across the 0.05-0.95 fraction sweep, producing the first per-method calibrated fractions used by all subsequent runs.
