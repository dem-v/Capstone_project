...
def main() -> None:
    ...
    with (args.output_dir / "run_selected_high_stability_diagnostics.ps1").open("w", encoding="utf-8") as handle:
        ...
        handle.write("$runStart = Get-Date\n")
        handle.write(f"$totalCases = {len(selected)}\n\n")
        handle.write("function Format-Duration([TimeSpan]$duration) {\n")
        handle.write("    if ($duration.TotalHours -ge 1) { return ('{0:0.0}h' -f $duration.TotalHours) }\n")
        handle.write("    return ('{0:0.0}m' -f $duration.TotalMinutes)\n")
        handle.write("}\n\n")
        handle.write("function Invoke-ReviewCase([int]$Index, [string]$Label, [scriptblock]$Command) {\n")
        handle.write("    $caseStart = Get-Date\n")
        handle.write("    $completed = $Index - 1\n")
        handle.write("    if ($completed -gt 0) {\n")
        handle.write("        $elapsed = $caseStart - $runStart\n")
        handle.write("        $avgSeconds = $elapsed.TotalSeconds / $completed\n")
        handle.write("        $eta = [TimeSpan]::FromSeconds($avgSeconds * ($totalCases - $completed))\n")
        handle.write("        Write-Host ('[{0}/{1}] START {2} | elapsed {3} | ETA {4}' -f $Index, $totalCases, $Label, (Format-Duration $elapsed), (Format-Duration $eta))\n")
        handle.write("    } else {\n")
        handle.write("        Write-Host ('[{0}/{1}] START {2}' -f $Index, $totalCases, $Label)\n")
        handle.write("    }\n")
        handle.write("    & $Command\n")
        handle.write("    if ($LASTEXITCODE -ne 0) { throw \"Case $Index failed with exit code ${LASTEXITCODE}: $Label\" }\n")
        handle.write("    $caseElapsed = (Get-Date) - $caseStart\n")
        handle.write("    $runElapsed = (Get-Date) - $runStart\n")
        handle.write("    $avgSeconds = $runElapsed.TotalSeconds / $Index\n")
        handle.write("    $eta = [TimeSpan]::FromSeconds($avgSeconds * ($totalCases - $Index))\n")
        handle.write("    Write-Host ('[{0}/{1}] DONE  {2} | case {3} | elapsed {4} | ETA {5}' -f $Index, $totalCases, $Label, (Format-Duration $caseElapsed), (Format-Duration $runElapsed), (Format-Duration $eta))\n")
        handle.write("}\n\n")
        for row in selected:
            label = f"{row['category']} | {row['filename']}"
            handle.write(f"# {row['review_rank']}. {label}\n")
            handle.write(
