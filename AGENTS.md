...
## Basic rules of interaction, research and quality control
- you are not my assistant. You are my advisor who happens to be smarter than me
- never start with agreement. challenge my assumptions, point out what is missing or ask questions to expose gap in my thinking
- rate your confidence, tag it [Certain] if you have hard eveidence, [Likely] if it is a strong inference, [Guessing] if you are filling gaps. 
- If most of your reply is guessing, say so first.
- kill these phrases: "Great question", "You're absolutely right", "That makes a lot of sense", "Absolutely", "Definitely". If you catch yourself typing one, delete and rewrite.
...
- give me uncomfortable answer first. If there is a truth I probably don't want to hear, lead with it
- No warm up paragraphs. Skip "There are several ways to look at this". Start with the most useful thing you can say
- If I push back, don't fold. Hold your position unless I give or you find genuinely new information. "But I really think" is not new information.
...
- For commands that need project-root certainty, use `wsl.exe --cd <REPO_ROOT> python3 ...`, where `<REPO_ROOT>` is the WSL path to this clone (on the original machine `/mnt/c/Users/Dmytro.Valantsevych/Downloads/master_thesis_draft_explainAI`; replace with your own clone path).
...
- Final thesis should reference AI/development tools and exact versions where applicable, including `GPT-5.5`, `Codex`, `PyCharm`, `Junie`, `VS Code`, `Claude Sonnet 4.6`, and `Claude Opus 4.7`, plus all research models and XAI methods actually used. Verify these exact tool names and version strings against what was actually used before they enter the signed Declaration of Academic Integrity; an inaccurate declaration is worse than an incomplete list.
- Tex documents are considered final thesis for publication and must be viewed from this perspective. All sections must be written in a way that is suitable for publication. Avoid using AI-talk style, internal notes, paths, references, and documentations that should not get into the paper.
...
Versioning discipline: v1 outputs are never overwritten or renamed. v2/v3 outputs live alongside v1 in new `outputs/iter_XX_*` folders. Held-out evaluation used v2 between Phase 1.2 and Phase 5.1; after Phase 5.1 (Eigen-CAM/Score-CAM) it uses v3 (see the Calibration Rules section for the authoritative version).
...
- Secondary pipeline (off-the-shelf model approach decided 2026-05-18; implemented since): head CT intracranial hemorrhage under `src/explainai_thesis/ct/` and `scripts/run_ct_smoke.py`. The CT pipeline is implemented and has been run (see `outputs/iter_53_ct_smoke_*`, `outputs/iter_54_ct_improvement_*`, `outputs/iter_59_ct_visual_smoke`). CT uses HU windowing rather than 0-255 grayscale normalization, so faithfulness baselines were re-chosen for CT. Masks for CT come from a small student-annotated positive subset stored only in `data_local/`. No CT model fine-tuning is in scope; an off-the-shelf public CT classifier is used.
...
- Tooling: `scripts/build_review_workbook.py` (added 2026-05-18; implemented and used, e.g. `outputs/iter_28_review_workbook`, `outputs/iter_43/45/48_*_review_workbook`) generates a static review folder per smoke run under `<run>/review/`.
- Outputs: `index.html` (one card per case, embedded four-view overlay grid per method, inline scoring guide covering the two scored rubrics plus the seven-category failure taxonomy), `scores_template.csv` (prefilled `case_id` and `filename` columns), and `INSTRUCTIONS.md` (open `index.html` in a browser, fill `scores.csv` in an editor, save next to the template, do not edit the template itself).
...
- Calibration versioning (added 2026-05-18): the `SignedAttribution` refactor changes the underlying signed maps for Grad-CAM, IG, GradientSHAP, and Occlusion, which makes pre-refactor calibrated top-fractions statistically stale. After the refactor, a `v2` calibration was produced and stored at `outputs/iter_XX_calibration_v2/calibrated_thresholds_v2.csv` (see `outputs/iter_29_calibration_v2`). The `v1` files are never overwritten or renamed; all versions coexist. Downstream scripts pick the version explicitly via `--calibrated-fractions`. `v2` was the held-out calibration only between the refactor and Phase 5.1; once Eigen-CAM and Score-CAM landed in Phase 5.1 (2026-05-27) they changed the method panel, so a `v3` calibration supersedes `v2` for held-out evaluation (see `outputs/iter_49_densenet_calibration_v3`, `outputs/iter_50_resnet_calibration_v3`, `outputs/iter_57_chex_calibration_v3`). `v1` and `v2` are retained only as historical artifacts; current held-out runs use `v3`.
...
- Pre-mortem revision (2026-05-18): Stage A was *intended* to include one out-of-family external model from the start, not as a Stage C contingency. Rationale: the in-family TorchXRayVision DenseNet-121 weights share architecture, input size, normalization, and substantially overlapping training data (CheX, MIMIC, NIH, PadChest), so a uniform within-family result would only rule out within-family variance, not training-distribution mismatch. The external candidate was to be selected at implementation time and integrated through the `load_classifier(name)` seam. NOTE: this commitment was not fully met — the executed in-family set grew to six DenseNet weights plus the cross-architecture `resnet50-res512-all`, but no off-the-shelf out-of-family pneumothorax classifier was verified (see Diagnostic A/B Results).
- Stage B outcome classification (refined with external-model context):
  ...
  - A same-library but different-architecture model (e.g. `resnet50-res512-all` vs the DenseNet-121 weights) is materially better than the in-family weights: cross-architecture variance within the library is meaningful; that model is promoted to co-primary baseline. This is distinct from both the within-family-weight case and the out-of-family-external case.
...
- Stage B outcome classification: `resnet50-res512-all` is materially better than `densenet121-res224-all` in this Stage A aggregate but absolute localization remains weak across all tested weights. Per the Stage B refined decision rule, this is the **same-library, different-architecture model is materially better + cross-distribution-stable weak absolute localization** case (ResNet-50 is a different architecture from the DenseNet-121 weights, not a different DenseNet weight): ResNet-50 is promoted to co-primary baseline for thesis follow-up (Phase 5.5), while the broader thesis framing remains methodological. The out-of-family Stage A slot remains open because no off-the-shelf pneumothorax-classifier-with-explicit-head was verified.
- Current-state note (2026-06-14): the Diagnostic A/B Results above are frozen as of 2026-05-20 and have since been built upon — ResNet/CheX classifier evaluation, calibration, and improvement runs (`outputs/iter_55`–`iter_58`), the CT pipeline (`outputs/iter_53`/`iter_54`/`iter_59`), and thesis charts (`outputs/iter_61`). A HuggingFace ViT CXR-14 model (`taheera/vit-in1k-chestxray14`, `outputs/iter_60_taheera_vit_probe`) was structurally probed only — it is an explicitly throwaway, non-thesis feasibility check and does NOT fill the open out-of-family Stage A slot.
...
9. List of Charts (СПИСОК ДІАГРАМ) — bar charts, pie charts. Faithfulness-AUC bar chart and review-score distribution counts likely qualify. (Confirm the exact Ukrainian heading against the Neoversity template; it must differ from the graphs heading above.)
